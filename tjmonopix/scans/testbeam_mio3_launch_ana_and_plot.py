...
ANA_SCRIPT = os.path.join(SCRIPT_DIR, "testbeam_mio3_analyze.py")
PLOT_SCRIPT = os.path.join(SCRIPT_DIR, "testbeam_mio3_plot.py")
# Glob expression for input files (ends in DATE.h5, where date is YYYY-MM-DD_HH-MM-SS)
SCAN_FILES = "*[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]_[0-9][0-9]-[0-9][0-9]-[0-9][0-9].h5"
# re.sub args to convert input file name to output file name
SCAN_TO_ANA_FILES = (r'\.h5$', r'_interpreted.h5')
...
while True:
    ...
    for scan_file in glob.iglob(SCAN_FILES):
        ...
        if py2([ANA_SCRIPT, scan_file]).returncode:
            print(f"ERR Analysis failed: manually delete {ana_file} to retry")
            continue
        print(f"Plotting  {scan_file}")
        if py2([PLOT_SCRIPT, ana_file]).returncode:
            print(f"ERR Plotting failed: manually {ana_file} to retry")
