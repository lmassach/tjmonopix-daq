...
def get_timewalk_hist(hit_file, fout, show_plots=False):
    ...
    tlu_tdc_type=[("tdc", 'u8'), ("tdc_timestamp", 'u8'),
                            ("tlu", 'u8'), ("tlu_timestamp", 'u8'),('n_match','u4')]
    ...
    def correlate_tdc_tlu(tdc_data, tlu_data, data_out, upper_lim, lower_lim):
        ...
        while tdc_i < len(tdc_data) and dat_i < len(data_out) and tlu_i < len(tlu_data):
            ...
            # If we get here, TLU timestamp is not delayed more than upper_limit
                ...
                for tlu_ii in range(tlu_i+1,len(tlu_data)):
                     if tdc_data[tdc_i]["timestamp"] < tlu_data[tlu_ii]["timestamp"] - np.uint64(np.abs(lower_lim)):
                          break
                data_out[dat_i]["n_match"]= tlu_ii-tlu_i
...
if __name__ == "__main__":
    ...
    #hit_files = ["/sirrush/silab/Data_Testbeams/2018/20181604_tjmonopix_ELSA_hitorData/run_180418-135931/tjmonopix_180418-135931_hit.h5",
    #             "/sirrush/silab/Data_Testbeams/2018/20181604_tjmonopix_ELSA_hitorData/run_180418-150717/tjmonopix_180418-150717_hit.h5"]

    hit_files = ["/sirrush/silab/Data_Testbeams/2018/20181604_tjmonopix_ELSA_hitorData/run_180418-164032/tjmonopix_180418-164032_hit.h5"]
    # Sr90 data
    ...
    for hit_file in hit_files:
        data = get_timewalk_hist(hit_file, fout=hit_file[:-7]+"_tdc.h5",show_plots=True)
