...
if __name__ == "__main__":
    ...
    sets = [hit_files_1, hit_files_2, hit_files_3]

    sets=[["/sirrush/silab/Data_Testbeams/2018/20181604_tjmonopix_ELSA_hitorData/run_180418-164032/tjmonopix_180418-164032_hit.h5"]]

    for i, file_set in enumerate(sets):
        hist, edges = np.histogram(np.array([]), bins=np.arange(-10, 1010, 20))

        for hit_file in file_set:
            data = get_timewalk_hist(hit_file)
            tmp_hist, _ = np.histogram(data["tdc"], bins=np.arange(-10, 1010, 20))
            hist += tmp_hist

        mids = edges[:-1] + 0.5 * (edges[11] - edges[10])
        plt.step(mids, hist, label=labels[i])
    plt.show()
